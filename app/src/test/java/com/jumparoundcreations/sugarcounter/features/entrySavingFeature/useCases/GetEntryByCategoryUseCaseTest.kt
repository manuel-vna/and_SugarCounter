...
class GetEntryByCategoryUseCaseTest {
    ...
    @Test
    fun `invoke returns EntryFound when database returns a SugarEntry`() = runTest {
        // Arrange
        val categoryToSearch = "Chocolate"
        val expectedEntry = SugarEntry(
            id = 1,
    ...
    @Test
    fun `invoke returns NoEntryFound when database returns null`() = runTest {
        // Arrange
