...
class EntryListDisplayingViewModelTest {
    ...
    private val sampleEntry = SugarEntry(
        id = 1,
    ...
    private val sampleEntryGroup = EntryGroup(
        date = "2023-10-27",
    ...
    @Test
    fun `initialization loads data into states`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `OpenCardDetails updates state with selected entry`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `LoadEntryDataIntoCardDetails updates state with entry values`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `EditCategory updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `DismissCardDetails updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `DeleteEntry calls deleteEntryUseCase`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `ChangeSearchTextFieldText updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `FilterEntryListInHistory calls filterUseCase and updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `CloseSearchFieldAndClearText resets search state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `ReuseEntryForToday calls reuseEntryForTodayUseCase`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `EditGram updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `EditQuantity updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `ShowDeleteEntryConfirmation updates state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `ChangeSearchFieldShown toggles searchFieldShown state`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `EditEntryInDB calls editDatabaseEntryUseCase with correct parameters`() = runTest {
        initViewModel()
        advanceUntilIdle()
    ...
    @Test
    fun `groupEntriesPerDayCounter handles error and updates state`() = runTest {
        val errorMessage = "Network Error"
        every { getEntryGroupPerDayUseCase(any()) } returns flow {
